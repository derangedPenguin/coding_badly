...
statItems = ['Strength:','Dexterity:','Constitution','Intelligence:','Wisdom:','Charisma:']

'''
*species and race used interchangeable, exclusively species in code so far (02/24/23 - 10:04am)
...
def assignSpeciesTraits():
  ...
  if Species == 'Grung':
    ...
    Speed = 25
    extraSpeed = 25
    extraSpeedType = 'Climb'
#(weird) Half-elf, Changeling, warforged, Harengon, Owlin, Grung, Triton - just anythin with stat bonuses assigned here
#not (max(statHoldList) == 18 and ((Species == 'Human') or (Species == 'Triton' and (Class == 'Barbarian' or Class == 'Paladin' or Class == 'Fighter' or Class == 'Sorcerer' or Class == 'Bard' or Class == 'Warlock'))) or (max(statHoldList) >= 17 and Species == 'Kobold' and (Class == 'Monk' or Class == 'Rogue' or Class == 'Ranger'))) -> checks for stat error causes
def assignStats():
  global stre, dex, con, inte, wis, cha, Class, Species, statHoldList
  if not (max(statHoldList) == 18 and ((Species == 'Human') or (Species == 'Triton' and (Class == 'Barbarian' or Class == 'Paladin' or Class == 'Fighter' or Class == 'Sorcerer' or Class == 'Bard' or Class == 'Warlock'))) or (max(statHoldList) >= 17 and Species == 'Kobold' and (Class == 'Monk' or Class == 'Rogue' or Class == 'Ranger'))):
    ...
    statHoldList.remove(max(statHoldList)) #removes just used num, all primary stats are now assigned
    if Class == 'Barbarian':
      ...
      statHoldList[random.randint(0,len(statHoldList)-1)] += 1 #randomized secondary stat bonus for classes without a secondary stat
  else:
    #human affects all by 1, triton is stre, con, and cha by 1, kobold is dex by 2
      if Class == 'Wizard' or Class == 'Artificer':
